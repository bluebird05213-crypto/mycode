...
- 打开题目助手后，模型下拉框会从 DeepSeek API 读取当前账户可用模型；选择会保存在当前浏览器本机，下一次打开仍会保留。若暂时无法读取，会回退到常用模型名。
- 模型下拉框中的“本地模式”不会调用 DeepSeek API；也可以点击“关闭 DeepSeek”立即切换到本地解析，适合控制费用、断网或不希望上传当前题目时使用。
- “复制完整题目包”会优先复制可直接粘贴到 Codex 的文字和 HTML 图片；浏览器不允许图片剪贴板时，会复制文字并提示使用 HTML 下载包。
