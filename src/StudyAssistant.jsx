...
async function askDeepSeek(prompt,context){
 const images=await loadContextImages(context);const response=await fetch('http://127.0.0.1:3001/api/assistant',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({prompt,context,images})});
...
function StudyAssistant(){
 ...
 const ask=async text=>{const clean=text.trim();if(!clean)return;const context=currentContext();setMessages(old=>[...old,{role:'user',text:clean},{role:'assistant',text:'正在请求 DeepSeek 分析…',pending:true}]);setInput('');try{const answer=await askDeepSeek(clean,context);setMessages(old=>{const next=[...old];next[next.length-1]={role:'assistant',text:answer};return next})}catch(error){setMessages(old=>{const next=[...old];next[next.length-1]={role:'assistant',text:`DeepSeek 暂时不可用：${error.message}\n\n已切换本地提示：${replyFor(clean,context)}`};return next})}};
