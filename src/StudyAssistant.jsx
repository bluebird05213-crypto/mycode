...
function StudyAssistant(){
 const[open,setOpen]=useState(false),[input,setInput]=useState(''),[model,setModel]=useState(()=>localStorage.getItem('beisen-assistant-model-v1')||'deepseek-chat'),[models,setModels]=useState(['deepseek-chat','deepseek-reasoner']),[messages,setMessages]=useState([{role:'assistant',text:'我是本机学习助手。可以先问“为什么选这个”或“这题有什么技巧”。'}]);
 ...
 useEffect(()=>{if(!open)return;fetch('http://127.0.0.1:3001/api/models').then(response=>response.ok?response.json():null).then(data=>{if(Array.isArray(data?.models)&&data.models.length){setModels(data.models);if(!data.models.includes(model))setModel(data.models[0])}}).catch(()=>{})},[open]);
